...
THREE.Ocean = function(renderer, camera, scene, options) {
	...
	this.geometryResolution = optionalParameter(
	...
	LinearClampParams.minFilter = LinearClampParams.magFilter =
		THREE.LinearFilter;
	LinearClampParams.wrapS = LinearClampParams.wrapT =
		THREE.ClampToEdgeWrapping;

	var NearestClampParams = JSON.parse(JSON.stringify(BaseParams));
	NearestClampParams.minFilter = NearestClampParams.magFilter =
		THREE.NearestFilter;
	NearestClampParams.wrapS = NearestClampParams.wrapT =
		THREE.ClampToEdgeWrapping;

	var NearestRepeatParams = JSON.parse(JSON.stringify(BaseParams));
	NearestRepeatParams.minFilter = NearestRepeatParams.magFilter =
		THREE.NearestFilter;
	NearestRepeatParams.wrapS = NearestRepeatParams.wrapT =
		THREE.RepeatWrapping;

	var LinearRepeatParams = JSON.parse(JSON.stringify(BaseParams));
	LinearRepeatParams.minFilter = LinearRepeatParams.magFilter =
		THREE.LinearFilter;
	...
	this.screenQuad.rotation.set(-90, 0, -45);
...
THREE.Ocean.prototype.generateMesh = function() {
	...
	this.oceanMesh.rotation.set(-90, 0, -45);

	// this.oceanMesh.position.set(0, 0, -100);
};
...
THREE.Ocean.prototype.renderInitialSpectrum = function() {
	...
	this.renderer.render(
		this.scene,
		this.oceanCamera,
		this.initialSpectrumFramebuffer,
		true
	);
};
...
THREE.Ocean.prototype.renderWavePhase = function() {
	...
	if (this.initial) {
		...
		this.materialPhase.uniforms.u_phases.value = this.pingPhase
			? this.pingPhaseFramebuffer
			: this.pongPhaseFramebuffer;
	}
...
THREE.Ocean.prototype.renderSpectrumFFT = function() {
	...
	for (var i = 0; i < iterations; i++) {
		if (i === 0) {
			...
			subtransformProgram.uniforms.u_input.value =
				iterations % 2 === 0
